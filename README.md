...
- Investigate output determinism — model outputs currently vary somewhat between runs on identical input, which makes any single accuracy number a snapshot rather than a stable measurement. Worth testing lower/zero temperature settings, evaluating a different model known for more consistent structured-output behavior, and further constraining the schema (e.g., tighter enums, stricter field formats) to see how much variance can be engineered out versus how much is inherent to the model.
